...
# 安全提取 y（支持中英文）
if 'Recreational Value' in df_percep.columns:
    y_series = df_percep['Recreational Value']
elif '休闲价值' in df_percep.columns:
    y_series = df_percep['休闲价值']
else:
    print("⚠️ 未找到 'Recreational Value' 或 '休闲价值' 列，将默认使用第4列作为安全性标签")
...
# 清理 y
if 'Recreational Value' in df_percep_aligned.columns:
    y_series = df_percep_aligned['Recreational Value']
elif '休闲价值' in df_percep_aligned.columns:
    y_series = df_percep_aligned['休闲价值']
else:
...
# 标题
ax.set_title('SHAP Mean Absolute Value (Recreational Value)', fontsize=12, pad=20)
