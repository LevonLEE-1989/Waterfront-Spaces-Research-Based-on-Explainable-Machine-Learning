...
# 安全提取 y（支持中英文）
if 'Safety' in df_percep.columns:
    y_series = df_percep['Safety']
elif '安全性' in df_percep.columns:
    y_series = df_percep['安全性']
else:
    print("⚠️ 未找到 'Safety' 或 '安全性' 列，将默认使用第3列作为安全性标签")
...
# 清理 y
if 'Safety' in df_percep_aligned.columns:
    y_series = df_percep_aligned['Safety']
elif '安全性' in df_percep_aligned.columns:
    y_series = df_percep_aligned['安全性']
else:
...
# 标题
ax.set_title('SHAP Mean Absolute Value (Safety)', fontsize=12, pad=20)
